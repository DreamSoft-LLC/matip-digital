...
export default function WithdrawModal() {
    ...
    const onWithdraw = () => {
        ...
        axios.post(`${API_SERVER}/payment/withdraw`, {
            amount,
            wallet
        }, { headers: { 'auth-token': session_key } })
            .then(res => { alerts.success("Withdrawal request is beign processed") })
            .catch(err => { alerts.error(err.response.data) })
    }
                ...
                <div class="flex flex-col w-full">
                    ...
                    <div class="flex w-full px-10">
                        <div class="flex flex-row border rounded flex-1">
                            ...
                            <input value={amount} onChange={e => setAmount(e.target.value)} type="number" name="price" placeholder="20" class=" bg-grey-lighter text-grey-darker py-2 font-normal rounded text-grey-darkest border-grey-lighter rounded-l-none w-full" />
                        </div>
                    </div>
                    ...
                    <div class="flex w-full px-10">
                        <div class="flex flex-row border rounded flex-1">
                            ...
                            <input onChange={e=>setWallet(e.target.value)} type="text" name="wallet" placeholder={wallet || "bitcoin address"} class=" bg-grey-lighter text-grey-darker py-2 font-normal rounded text-grey-darkest border-grey-lighter rounded-l-none w-full" />
                        </div>
                    </div>
                    ...
                    <div class="p-3  mt-2 text-center space-x-4 md:block">
                        ...
                        <button onClick={e => onWithdraw()} class="mb-2 md:mb-0 bg-blue-500 border border-blue-500 px-5 py-2 text-sm shadow-sm font-medium tracking-wider text-white rounded-full hover:shadow-lg hover:bg-blue-600">Withdraw</button>
                    </div>
                </div>
            </div>
        </div>
